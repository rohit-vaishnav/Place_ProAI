...
function parseHSL(hslStr: string): { h: number; s: number; l: number } {
  // matches "h s l" or "h, s, l"
  const match = hslStr.match(/([\d.]+)\s*,?\s*([\d.]+)%?\s*,?\s*([\d.]+)%?/);
  if (!match) return { h: 105, s: 45, l: 75 }; // Default Soft Academic sage tint
...
const BorderGlow: React.FC<BorderGlowProps> = ({
  ...
  return (
    <div
      ...
      className={`relative grid isolate border border-neutral-200/40 ${className}`}
      style={{
        background: backgroundColor,
        borderRadius: `${borderRadius}px`,
        transform: 'translate3d(0, 0, 0.01px)',
        boxShadow: 'rgba(0,0,0,0.03) 0 1px 2px, rgba(0,0,0,0.03) 0 2px 4px, rgba(0,0,0,0.03) 0 4px 8px',
      }}
    >
